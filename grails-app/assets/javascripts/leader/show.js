...
;-function (window) {
    'use strict';
    var ready = {
        planId: $('.plan-info').attr('planId'),
        trialId: $('.plan-info').attr('trialId'),
        allowPlay: $('.plan-info').attr('allowPlay'),
    ...
    var init = function () {
        ...
        core.handleNote();//获取笔录
        core.initVideoData(); //加载视频数据
        // core.deleteMsg();//删除通讯消息
        // core.sendMsg();//发送通讯消息
        // core.chatScroll();//初始化聊天框的滚动条
        // core.videoPlay();
        core.onbeforeunload();
        core.selectpicker();
        // core.caseChange();//并案
    ...
    function init_event() {//初始化页面事件
        core.consoleBtn();//控制台按钮
        core.changeChn();//切换通道
        // core.importWord();//上传文件
    }
    ...
    var core = {
        ...
        //                 }, function () {
        //                     window.location.href = contextPath + "leader/list";
        //                 });
        //             }
        //         }, 'json'
        //     )
        ...
        //     });
        // },
        handleNote: function () {//尝试获取笔录文件
            $("#note").click(function () {
                var penDiv = $(".penhoder");
                // penDiv.html('<sec:ifAnyGranted roles=\'ROLE_SUPER,ROLE_ADMIN,ROLE_/plan/downTrialNote\'> <a href="' + contextPath + 'leader/downTrialNote/' + ready.trialId + '" class="btn btn-info">下载笔录</a> </sec:ifAnyGranted>');
                $.ajax({
                    type: "get",
                    url: contextPath + "leader/downTrialNote/" + ready.trialId,
                    dataType: "json",
                    success: function (result) {
                        penDiv.html(result.message);
                    }
                });
            ...
            // $.ajax({
            //     type: "post",
            //     url: contextPath + "leader/demo/" + ready.trialId,
            //     dataType: "json",
            //     success: function (result) {
            //         if (result.code === 0) {
        ...
        handleVideo: function () { //处理视频方法
            if (ready.data.status === 0) {//排期状态
                ...
                return
            }else if (ready.data.status === 1) { //开庭状态 直播模式
        ...
        handleVideoDom: function () {
            ...
            for (var j = 0; j < pvList.length; j++) {
                ...
                var videoItem = $('<li class="fa-video">' +
                    ...
                    '                      <a title="点击下载" href="'+ contextPath + "leader/download/?url=" + url +'" download="'+ pvList[j].startRecTime +  '.mp4">' +
                    '                          <img title="点击下载" src="'+ contextPath + 'leader/getComment?trialId=' + ready.trialId + '&picName=' + ready.chnNumber + ready.chnName + pvList[j].startRecTime.toString().replace(":","") + '.jpg" alt=""/>' +
                    '                      </a>' +
                    '                  </div>' +
        ...
        initVideoData: function () { //加载视频数据
            ...
            $.ajax({
                type: 'post',
                url: contextPath + 'leader/liveShow/' + ready.trialId,
                dataType: 'json',
                success: function (result) {
        ...
        consoleBtn: function () {
            $("#begincourt").click(function () {
                $.get(
                    contextPath + 'plan/combinedCheck',
                    {
                        planId: ready.planId
                    },
                    function (result) {
            ...
            // $("#TrialNode").click(function () {
            //     $.get(
            //         contextPath + 'plan/uploadConsoleNode',
            //         {
            //             id: ready.trialId
            //         },
            //         function (result) {
        ...
        confirmPlan: function () {
            swal({
                ...
                $.get(
                    contextPath + 'plan/editCombinedPlanOpen',
                    {
                        planId: ready.planId,
                        trialId: ready.trialId
                    },

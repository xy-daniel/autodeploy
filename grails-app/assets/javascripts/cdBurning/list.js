    ...
    var core = {
                ...
                swal({
                    ...
                    for (var i=0; i<checkedBox.length; i++){
                        if (checkedBox[i].checked){
                            if (cdBurningIds==""){
                                cdBurningIds = checkedBox[i].value;
                            }else{
                                cdBurningIds = cdBurningIds + "," + checkedBox[i].value;
                            }
                        }
                    }
                    $.get(
                        contextPath + 'cdBurning/del',
                        {
                            cdBurningIds:cdBurningIds
                        },
                        function (result) {
                            if (result.code==0){
                                swal({
        ...
        render_table: function () {
            $('#data-table').DataTable({
                ...
                bProcessing: true,
                ajax: {
                    url: contextPath + 'cdBurning/list',
                    type: "POST"
                },
                aoColumns: [//初始化要显示的列
                    //id
                    ...
                    //对该用户的操作
                    {
                        ...
                        "mRender": function (data, type, full) {//返回自定义的样式
                            ...
                            html +=  '<a href="'+contextPath+'cdBurning/edit/' + data + '" class="btn btn-inverse btn-xs m-r-5 btn-enabled">编辑</a>';
                            html +=  '</div>';
                            return html
                        }
                    }
